...
class customer extends MY_Controller{
	...
	public function listall(){
		...
		$config['base_url'] = base_url().'admin/customer/listall?';
		//config phân trang
		$config['total_rows'] = $this->muser->numRowsByType('member');
		$config['per_page'] = 8;
		...
		$this->_data['template'] = 'quanly/customer/list_view';
		$this->_data['title'] = 'Trang Quản Lý User ';
		$this->_data['info'] = $this->muser->listAllByType($config['per_page'], $start, 'member');
	...
	public function add(){
		...
		if(isset($_POST['ok'])){
            ...
            if($this->form_validation->run()==TRUE){
				...
				redirect(base_url()."quanly/customer/listall");
			}
            $arr = array(
                'user_name' => $this->input->post('user_name'),
				'name' => $this->input->post('name'),
		...
		$this->_data['template'] = 'quanly/customer/modify_view';
